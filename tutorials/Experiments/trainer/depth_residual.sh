...
export JOB_NUM=20
# Utility
export experiment_name="depth_residual_04"
...
base_funs=('silu' 'zero')
depths=(50 45 40 35 30 25 20 15 10 5 1)
...
first_execution=true
echo "DEPTH RUNS"
experiment_name_mod="${experiment_name}"
for base_fun in "${base_funs[@]}"; do
    for hidden_depth in "${depths[@]}"; do
        toggle_device_index=$((index % 2))
        echo "Running with dataset: $dataset, base_fun: $base_fun, hidden_depth: $hidden_depth, hidden_width: $hidden_width, grid: $grid on device_index: $toggle_device_index"
